...
def res_conv_block(x, filter_size, size, dropout, batch_norm=False):
    ...
    shortcut = conv = layers.Conv2D(size, (filter_size, filter_size), padding="same")(x)
    if batch_norm:
...
def attention_block(x, gating, inter_shape):
    ...
    upsample_psi = layers.UpSampling2D(size=(2, 2))(sigmoid_xg)  # 32
...
def attention_resunet(n_classes=5, dropout_rate=0.2, batch_norm=True):
    ...
    inputs = layers.Input((None, None, 3), dtype=tf.float32)
    axis = 3
